...
def read_cellcounter_xml(source):
    """
    Reads XML file generated by the ImageJ plugin CellCounter into two pandas dataframes
    
    Parameters
    ----------
    - source : string or file object
        Source is a filename or file object containing XML data, accepted as input by
        xml.etree.ElementTree.parse.

    Returns
    -------
    - cc_data : dictionary of dataframes:
        'image_props' : pandas dataframe
        `image_props` contains the image for which markers were found in ImageJ-CellCounter.
        It will contain a single row and the following columns:
        > Image_Filename: name of the image file initialised within CellCounter.
        > X_Calibration: pixel size in the X axis (usually the same as the Y axis)
    ...
    xmldic = xtd.parse(xmlstr)    
    if (len(xmldic)==1) and \
        (list(xmldic.keys())[0]=='CellCounter_Marker_File'):
        xmldic = xmldic['CellCounter_Marker_File']
    else:
        raise ValueError('The XML file you used does not seem to be a CellCounter Marker File')

    image_props = pd.DataFrame.from_dict([ xmldic['Image_Properties'] ])
    cols = [x for x in image_props if x.endswith('_Calibration')]
    image_props[cols] = image_props[cols].apply(pd.to_numeric)
    
    columns = ['Type'] + list(xmldic['Marker_Data']['Marker_Type'][0]['Marker'][0].keys())
    marker_pos = pd.DataFrame(columns = columns)
    for m in xmldic['Marker_Data']['Marker_Type']:
        if 'Marker' in m.keys():
            if isinstance(m['Marker'], dict):
                dummy = pd.DataFrame.from_dict( [ m['Marker'] ] )
            else:
                dummy = pd.DataFrame.from_dict( m['Marker'] )
            dummy['Type'] = m['Type']
            marker_pos = pd.concat(
                [marker_pos if not marker_pos.empty else None,
                 dummy],
                ignore_index=True
            )
    marker_pos = marker_pos.apply(pd.to_numeric)
    
    return {'image_props' : image_props, 'marker_pos' : marker_pos}
